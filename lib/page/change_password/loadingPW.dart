...
class loadingChangePWScreen extends ConsumerStatefulWidget {
  const loadingChangePWScreen({super.key});

  @override
  ConsumerState<loadingChangePWScreen> createState() => _loadingScreenState();
}

class _loadingScreenState extends ConsumerState<loadingChangePWScreen> {
  @override
  void initState() {
    super.initState();
    getOTP();
  ...
  Future<void> sendData(String OTP) async {
    try {
      //print("personCode1 ${SharedPrefs.read(personCode)}");
      final data =
          '{"personCode": "${SharedPrefs.read(personCode)}","userCode": "${SharedPrefs.read(userCode)}", "otpCode": "$OTP", "applicationId": 2}';

      final encryptedBody = Aes256.encrypt(data, SharedPrefs.read(totp));
      print("encryptedBody MPIN $encryptedBody");
      http.Response response = await http.post(
        Uri.parse(DigiCoopAPI.updatePW),
        body: {'data': encryptedBody},
      );
      // Parse the JSON response body
      final responseData = json.decode(response.body);
      ...
      if (response.statusCode == 201) {
        context.pushNamed(changePWSuccess);
      } else if (response.statusCode == 400) {
        Flush.flushMessage(
          icons: Icons.error_outline,
          title: "Error",
